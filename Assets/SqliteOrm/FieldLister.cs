				...
				if(prop.CanRead && prop.CanWrite){
					...
					if(info.GetAttribute<IgnoreAttribute>() == null){
						...
						if(keyAttribute != null){
							if (classInfo.KeyField == null)
							  classInfo.KeyField = info;
							else
								classInfo.KeyField2 = info;
...
//				desc.NameInTable = desc.Name.ToLower();
	...
	public class FieldDesc{
      ...
      /*
      int a;
      if (obj.ToString().Contains( "Weapon" ) && obj.ToString().Contains( "List" ))
        a = 1;
      */
      if (Prop != null) {
        //Debug.Log( "Prop.GetValue( obj, new object[0] ) " + Prop.GetValue( obj, new object[0] ) );
        return Prop.GetValue( obj, new object[0] );
      }
      else {
        //Debug.Log( "Field.GetValue( obj ) " + Field.GetValue( obj ) );
