...
namespace UnityORM
{
    ...
    public string GenerateDeleteSQL<T>( ClassDesc desc, string column, object value ) {
      if (!desc.FieldDescs.Any( f => f.Name == column )) throw new Exception( $"Class {desc.Name} hasn't field named {column}" );
      return "DELETE FROM " + desc.Name + " WHERE " + column + " = " + ValueToBlock( value ) + ";";
    }
    ...
    public string GenerateDeleteSQL<T>(ClassDesc desc,object key){
		...
		public string GenerateInsertSQLHead<T>( ClassDesc desc, T obj ) {
			var builder = new StringBuilder();
			builder.Append( "INSERT INTO " + desc.Name + " (" );
		...
		public string GenerateInsertSQLSelect<T>( ClassDesc desc, T obj ) {
			var builder = new StringBuilder();
		...
		public string GenerateUpdateSQL<T>(ClassDesc desc,T obj){
			...
			if (desc.KeyField2 != null)
				builder.Append( " AND " + desc.KeyField2.NameInTable + " = " + ValueToBlock( desc.KeyField2.GetForDb( obj ) ) );
