...
public class Play_RZ_02 : UnitPlayWeaponBase {
  ...
  public override void SetupWeapon() {
    ...
    riflePoint = transform.Find("Armature/Hip/Waist/Body/Chest/ChestCannonPos");
    ...
    CoroutineCommon.CallWaitForAnimation( myAnimation, () => {
      ...
      CoroutineCommon.CallWaitForAnimator( animator, "MeleeAttack_" + sideDir, () => {
        ...
        CoroutineCommon.CallWaitForAnimator( animator, "MeleeAttack2_" + sideDir, () => {
          ...
          playGetHitCommon( 4 / 25f, true, new List<HitData>() { new HitData { WaitTime = 0f, DelayTime = 2f, Damage = attData.TotalDamage, HitEffect = "SaberHit" } }, BaseCallback );
        } );
  ...
  public void playWeapon3() {
    ...
    int row = 3;
    int col = 4;
    float totalTime = (loop * (row * col) - 1) * missileRate;
    float delayTime = totalTime / 3;

    myAnimation.Stop();
    myAnimation["weapon3"].speed = 1f;
    myAnimation.Play( "weapon3" );

    EffectSoundController.PLAY( (AudioClip)Resources.Load( "SFX/Weapon/Funnel" ), 5 );  //第一次播放音效
    CoroutineCommon.CallWaitForSeconds( .5f, () => {
      EffectSoundController.PLAY( (AudioClip)Resources.Load( "SFX/Weapon/Funnel" ), 5 );  //第二次播放音效
      damageController.PlayEn( attData.FromUseEN );     //扣EN
    } );
    CoroutineCommon.CallWaitForSeconds( .7f, () => { EffectSoundController.PLAY( (AudioClip)Resources.Load( "SFX/Weapon/Funnel" ), 5 ); } );  //第三次播放音效

    CoroutineCommon.CallWaitForAnimation( myAnimation, () => {    //動畫結束後
      switchCam();
      ...
      playMissileTo( 1f,
        new List<HitData>() {
            new HitData { WaitTime = 0, DelayTime = delayTime, Damage = (int)(attData.TotalDamage * .33f) },
            new HitData { WaitTime = 0, DelayTime = delayTime, Damage = (int)(attData.TotalDamage * .66f) },
            new HitData { WaitTime = 0, DelayTime = delayTime + .3f, Damage = attData.TotalDamage }
        },
        loop, row, isCollider: false,
        speed: 15, customPf: "Battle/Weapons/FunnelKnife" );
    } );
