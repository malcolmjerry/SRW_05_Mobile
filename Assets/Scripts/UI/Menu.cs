...
public class Menu : MonoBehaviour {
  ...
  public List<CmdEnum> CmEnumList;
  ...
  public UnitCommand[] cmdTextList = {
    new UnitCommand () { index = CmdEnum.MOVE,  text = "移動" },
    new UnitCommand () { index = CmdEnum.ATK  , text = "攻擊" },
    new UnitCommand () { index = CmdEnum.SPCMD, text = "精神" },
    new UnitCommand () { index = CmdEnum.INFO,  text = "能力" },
    new UnitCommand () { index = CmdEnum.ACTION_END,  text = "待機" },
    new UnitCommand () { index = CmdEnum.PHASE_END,  text = "回合終了" },
    new UnitCommand () { index = CmdEnum.OBJECTIVE,  text = "作戰目的" },
    new UnitCommand () { index = CmdEnum.TEAMLIST,  text = "味方部隊表" },
    new UnitCommand () { index = CmdEnum.ENEMYLIST,  text = "敵方部隊表" },
    new UnitCommand () { index = CmdEnum.BACK_TO_MAIN,  text = "回到主選單" },
    new UnitCommand () { index = CmdEnum.MAPSAVE,  text = "SAVE" },
    new UnitCommand () { index = CmdEnum.MAPLOAD,  text = "LOAD" },
    new UnitCommand () { index = CmdEnum.ON_SHIP,  text = "搭載" },
    new UnitCommand () { index = CmdEnum.OFF_SHIP,  text = "發進" }
  };
  public enum CmdEnum { MOVE = 0, ATK, SPCMD, INFO, ACTION_END, PHASE_END, TEAMLIST, ENEMYLIST,
    OBJECTIVE, BACK_TO_MAIN, MAPSAVE, ON_SHIP, OFF_SHIP, MAPLOAD }   //MAPLOAD為測試用, 之後要移除
  ...
  public void createCommands( int commandWidth, Action backAction ) {
    ...
    bkgRect.sizeDelta = new Vector2( cmdWidth + 10, CmEnumList.Count * selRectHeight + 10 );

    clearPreviousCmd();

    cmdList = new GameObject[CmEnumList.Count];
    float topY = ((cmdList.Length - 1)/2f) * selRectHeight;
    for (int i = 0; i < CmEnumList.Count; i++) {
      ...
      UnitCommand unitComm = cmdTextList.FirstOrDefault( c => c.index == CmEnumList[i] );
