...
public class GeroBeam : MonoBehaviour {
  ...
  void Update() {
    ...
    if (Width >= 0.01f) {
      for (int i = 0; i < LRSize; i++) {
        ...
        /*
        if (false && HitEffect != null && Physics.Raycast( NowPos, F_Vec[i], out hit, BlockLen * workNLG, layerMask )) {
  ...
  float scaleSpeed;
  Action callBack;
  public void SmoothDestroy( Action _callBack ) {
    callBack = _callBack;
    originScale = BP.Scale;
    scaleSpeed = originScale * (2/60f);

    DoSmoothDestroy();
  }

  private void DoSmoothDestroy() {
    CoroutineCommon.CallWaitForSeconds( 1/60f, () => {
      BP.Scale -= scaleSpeed;
